...
public:
    ...
    car(const std::string &md, const std::string &mk, int dp, int p, int w) : m_model(md), m_maker(mk), m_displacement(dp), m_ps(p), m_weight(w) {}

    ~car() {}

    std::string model() const { return m_model; } // 名前を返す
...
public:
    ...
    hybrid_car(const std::string &md, const std::string &mk,
               int dp, int p, int w, int mps) : car(md, mk, dp, p, w),
                                                m_motor_ps(mps) {}
    ...
    int tax() const { return 0; } // 「税金がタダ」(後で書き換える
};
